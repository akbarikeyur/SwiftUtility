//
//  MyUIView+MyKeyboardToolbar.swift
//  SwiftUtility
//
//  Created by Keyur.
...
private var kMyShouldHideToolbarPlaceholder = "kMyShouldHideToolbarPlaceholder"
private var kMyToolbarPlaceholder           = "kMyToolbarPlaceholder"

private var kMyKeyboardToolbar              = "kMyKeyboardToolbar"

/**
 MyBarButtonItemConfiguration for creating toolbar with bar button items
 */
@objc public class MyBarButtonItemConfiguration: NSObject {
    ...
    @objc public let action: Selector?  //action for bar button item. Usually 'doneAction:(MyBarButtonItem*)item'.
}
...
/**
UIView category methods to add MyToolbar on UIKeyboard.
*/
@objc public extension UIView {
    ...
    /**
     MyToolbar references for better customization control.
     */
    @objc var keyboardToolbar: MyToolbar {
        var toolbar = inputAccessoryView as? MyToolbar
        
        if toolbar == nil {
            toolbar = objc_getAssociatedObject(self, &kMyKeyboardToolbar) as? MyToolbar
        }
        ...
        if let unwrappedToolbar = toolbar {
            ...
            let newToolbar = MyToolbar(frame: frame)
            
            objc_setAssociatedObject(self, &kMyKeyboardToolbar, newToolbar, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
    ...
    */
    @objc var shouldHideToolbarPlaceholder: Bool {
        get {
            let aValue = objc_getAssociatedObject(self, &kMyShouldHideToolbarPlaceholder) as Any?
        ...
        set(newValue) {
            objc_setAssociatedObject(self, &kMyShouldHideToolbarPlaceholder, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
    ...
    @objc var toolbarPlaceholder: String? {
        get {
            let aValue = objc_getAssociatedObject(self, &kMyToolbarPlaceholder) as? String
            
            return aValue
        }
        set(newValue) {
            objc_setAssociatedObject(self, &kMyToolbarPlaceholder, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
    ...
    @objc var drawingToolbarPlaceholder: String? {
        ...
        if self.shouldHideToolbarPlaceholder {
            ...
            } else if let textView = self as? MyTextView {
    ...
    private static func flexibleBarButtonItem () -> MyBarButtonItem {
        
        struct Static {
            
            static let nilButton = MyBarButtonItem(barButtonSystemItem: .flexibleSpace, target: nil, action: nil)
        }
    ...
    @objc func addKeyboardToolbarWithTarget(target: AnyObject?, titleText: String?, rightBarButtonConfiguration: MyBarButtonItemConfiguration?, previousBarButtonConfiguration: MyBarButtonItemConfiguration? = nil, nextBarButtonConfiguration: MyBarButtonItemConfiguration? = nil) {
            ...
            if let prevConfig = previousBarButtonConfiguration {
                ...
                if prevConfig.barButtonSystemItem == nil && prev.isSystemItem == false {
                    ...
                    if let systemItem = prevConfig.barButtonSystemItem {
                        prev = MyBarButtonItem(barButtonSystemItem: systemItem, target: target, action: prevConfig.action)
                        prev.isSystemItem = true
                    } else if let image = prevConfig.image {
                        prev = MyBarButtonItem(image: image, style: .plain, target: target, action: prevConfig.action)
                    } else {
                        prev = MyBarButtonItem(title: prevConfig.title, style: .plain, target: target, action: prevConfig.action)
                    }
            ...
            if let nextConfig = nextBarButtonConfiguration {
                ...
                if nextConfig.barButtonSystemItem == nil && next.isSystemItem == false {
                    ...
                    if let systemItem = nextConfig.barButtonSystemItem {
                        next = MyBarButtonItem(barButtonSystemItem: systemItem, target: target, action: nextConfig.action)
                        next.isSystemItem = true
                    } else if let image = nextConfig.image {
                        next = MyBarButtonItem(image: image, style: .plain, target: target, action: nextConfig.action)
                    } else {
                        next = MyBarButtonItem(title: nextConfig.title, style: .plain, target: target, action: nextConfig.action)
                    }
            ...
            if let rightConfig = rightBarButtonConfiguration {
                ...
                if rightConfig.barButtonSystemItem == nil && done.isSystemItem == false {
                    ...
                    if let systemItem = rightConfig.barButtonSystemItem {
                        done = MyBarButtonItem(barButtonSystemItem: systemItem, target: target, action: rightConfig.action)
                        done.isSystemItem = true
                    } else if let image = rightConfig.image {
                        done = MyBarButtonItem(image: image, style: .plain, target: target, action: rightConfig.action)
                    } else {
                        done = MyBarButtonItem(title: rightConfig.title, style: .plain, target: target, action: rightConfig.action)
                    }
    ...
    @objc func addDoneOnKeyboardWithTarget(_ target: AnyObject?, action: Selector, titleText: String?) {
        ...
        let rightConfiguration = MyBarButtonItemConfiguration(barButtonSystemItem: .done, action: action)
    ...
    @objc func addRightButtonOnKeyboardWithImage(_ image: UIImage, target: AnyObject?, action: Selector, titleText: String?) {
        ...
        let rightConfiguration = MyBarButtonItemConfiguration(image: image, action: action)
    ...
    @objc func addRightButtonOnKeyboardWithText(_ text: String, target: AnyObject?, action: Selector, titleText: String?) {
        ...
        let rightConfiguration = MyBarButtonItemConfiguration(title: text, action: action)
    ...
    @objc func addCancelDoneOnKeyboardWithTarget(_ target: AnyObject?, cancelAction: Selector, doneAction: Selector, titleText: String?) {
        ...
        let leftConfiguration = MyBarButtonItemConfiguration(barButtonSystemItem: .cancel, action: cancelAction)
        let rightConfiguration = MyBarButtonItemConfiguration(barButtonSystemItem: .done, action: doneAction)
    ...
    @objc func addRightLeftOnKeyboardWithTarget(_ target: AnyObject?, leftButtonTitle: String, rightButtonTitle: String, leftButtonAction: Selector, rightButtonAction: Selector, titleText: String?) {
        ...
        let leftConfiguration = MyBarButtonItemConfiguration(title: leftButtonTitle, action: leftButtonAction)
        let rightConfiguration = MyBarButtonItemConfiguration(title: rightButtonTitle, action: rightButtonAction)
    ...
    @objc func addRightLeftOnKeyboardWithTarget(_ target: AnyObject?, leftButtonImage: UIImage, rightButtonImage: UIImage, leftButtonAction: Selector, rightButtonAction: Selector, titleText: String?) {
        ...
        let leftConfiguration = MyBarButtonItemConfiguration(image: leftButtonImage, action: leftButtonAction)
        let rightConfiguration = MyBarButtonItemConfiguration(image: rightButtonImage, action: rightButtonAction)
    ...
    @objc func addPreviousNextDoneOnKeyboardWithTarget (_ target: AnyObject?, previousAction: Selector, nextAction: Selector, doneAction: Selector, titleText: String?) {
        ...
        let rightConfiguration = MyBarButtonItemConfiguration(barButtonSystemItem: .done, action: doneAction)
        let nextConfiguration = MyBarButtonItemConfiguration(image: UIImage.keyboardNextImage() ?? UIImage(), action: nextAction)
        let prevConfiguration = MyBarButtonItemConfiguration(image: UIImage.keyboardPreviousImage() ?? UIImage(), action: previousAction)
