...
open class MyBarButtonItem: UIBarButtonItem {

    private static var _classInitialize: Void = classInitialize()

    @objc public override init() {
        _ = MyBarButtonItem._classInitialize
          super.init()
      }
    ...
    @objc open func setTarget(_ target: AnyObject?, action: Selector?) {
        if let target = target, let action = action {
            invocation = MyInvocation(target, action)
        } else {
            invocation = nil
        }
    }
     ...
     */
    @objc open var invocation: MyInvocation?
